  ...
  try {
    switch (val) {
      ...
      case 'reset':
        cloakTitle('reset');
        cloakFavicon('reset');
        notification('Presets reset to default.', '#95ff8a');
        break;
      default:
        notification('No preset selected.', '#ff9999');
    }
  } catch (err) {
...
function cloakTitle(newTitle) {
  try {
    if (newTitle === 'reset') {
      ...
      notification('Title reset to default.', '#95ff8a');
    } else if (!newTitle.trim()) {
      throw new Error('Title cannot be empty');
    } else {
      document.title = newTitle;
      localStorage.setItem('customTitle', newTitle);
      notification('Title set to "' + newTitle + '".', '#95ff8a');
    }
  } catch (err) {
    notification('Failed to set title: ' + err.message, '#ff9999');
  }
}
...
function handleInviteCode() {
  ...
  if (inviteMatch) {
    ...
    notification(`Welcome! You've been invited with code: ${inviteCode}`, "#95ff8a");
    
    // Store the invite code in localStorage for potential account creation benefit
    localStorage.setItem('inviteCode', inviteCode);
    
    // Redirect to main page after showing notification
    setTimeout(() => {
      console.log('Redirecting from invite code page to main page');
      
      // Clear the invite URL from browser history
      window.history.replaceState({}, '', '/');
      
      // Wait a moment for DOM to be ready, then navigate to landing page
      setTimeout(() => {
        const iframe = document.getElementById("frame" + currentTab);
        if (iframe) {
          console.log('Setting iframe src to landing page');
          iframe.src = '/subpages/landing/l.html';
        } else {
          console.log('Iframe not found, reloading page');
          // If iframe doesn't exist, reload the page to start fresh
          window.location.href = '/';
        }
      }, 100);
    }, 2000); // Reduced delay to prevent user confusion
  }
}

let currentTab = 0;
let tabs = [{ url: 'about:blank', history: [], currentHistoryIndex: -1 }];
let searchBar, urlInput, contextMenu, addTabButton;

// Initialize DOM elements after page loads
document.addEventListener("DOMContentLoaded", () => {
  searchBar = document.getElementById('searchBar');
  urlInput = document.getElementById('searchBar');
  contextMenu = document.getElementById('contextMenu');
  addTabButton = document.querySelector('.add-tab');
});

// Wait for DOM to be ready before setting up iframe event listeners
document.addEventListener("DOMContentLoaded", () => {
  const currentFrame = document.getElementById("frame" + currentTab);
  if (currentFrame) {
    currentFrame.addEventListener('load', () => {
      try {
        const currentURL = currentFrame.contentWindow.location.href;
        ...
        if (tabElement && tabElement.querySelector('p')) {
          tabElement.querySelector('p').textContent = currentFrame.contentDocument.title;
        }
        ...
        logWebsiteVisit(currentFrame.contentWindow.location.href);
      } catch (error) {
        console.log('iframe load event error (this is normal for cross-origin content):', error.message);
      }
    });
  }
});
...
async function runService(url) {
  ...
  if (tab.url === 'helium://settings' || tab.url.includes("/subpages/settings/s.html")) {
    ...
    // Use bare server instead of WISP for improved compatibility
    ...
    if (/\/class\//.test(tab.url)) {
      ...
      if (await connection.getTransport() !== "/bare/") {
        try {
          await connection.setTransport("/bare/", ["/bare/"]);
        } catch (error) {
          console.warn("Failed to set bare transport, using default:", error);
        }
      }
      ...
      // Use bare server transport instead of WISP
      if (await connection.getTransport() !== "/bare/") {
        try {
          await connection.setTransport("/bare/", ["/bare/"]);
        } catch (error) {
...
// Move searchBar event listener to DOM ready handler
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (searchBarElement) {
    searchBarElement.addEventListener("keydown", function (e) {
      if (e.key === "Enter") {
        e.preventDefault();
        runService(searchBarElement.value);
        setTimeout(() => {
          ...
          const suggestionsBackground = document.getElementById('suggestionsBackground');
          if (suggestionsBackground) {
            suggestionsBackground.style.display = 'none';
          }
          searchBarElement.style.borderBottomLeftRadius = "3px";
          searchBarElement.style.borderBottomRightRadius = "3px";
          const searchbarBg = document.getElementById("searchbarbackground");
          if (searchbarBg) {
            searchbarBg.style.borderBottomRightRadius = "3px";
            searchbarBg.style.borderBottomLeftRadius = "3px";
          }
        }, 200);
      }
    });
...
// Move all search bar event listeners to DOM ready handler
document.addEventListener("DOMContentLoaded", () => {
  const suggestionsList = document.getElementById('suggestions');
  const searchBarElement = document.getElementById('searchBar');
  ...
  if (searchBarElement && suggestionsList) {
    searchBarElement.addEventListener("focus", () => {
      searchBarElement.select();
      if (!(searchBarElement.value.trim() === "")) {
        if (suggestionsBackground) {
          suggestionsBackground.style.height = suggestionsList.offsetHeight + 'px';
          suggestionsBackground.style.display = 'block';
        }
        ...
        if (searchbarBackground) {
          searchbarBackground.style.borderBottomRightRadius = "0px";
          searchbarBackground.style.borderBottomLeftRadius = "0px";
        }
        const script = document.createElement('script');
        script.src = `https://suggestqueries.google.com/complete/search?client=firefox&q=${searchBarElement.value.trim()}&callback=handleSuggestions`;
    ...
    searchBarElement.addEventListener("blur", () => {
      setTimeout(() => {
        ...
        searchBarElement.style.borderBottomRightRadius = "3px";
        if (searchbarBackground) {
          searchbarBackground.style.borderBottomRightRadius = "3px";
          searchbarBackground.style.borderBottomLeftRadius = "3px";
        }
      }, 200);
    });
...
// Move suggestions list click event listener to DOM ready handler
document.addEventListener("DOMContentLoaded", () => {
  const suggestionsList = document.getElementById('suggestions');
  if (suggestionsList) {
...
async function login(username, password) {
  try {
    ...
    // Send plain password - server will hash and compare
    ...
    if (response.ok) {
      ...
      notification("Successfully logged in!", "#95ff8a");
      location.reload();
    } else {
      document.getElementById('loginError').style.display = "block";
...
async function createReferralLink() {
  ...
  if (!username) {
      notification("You must be logged in to create a referral link.", "#ff9999");
      return;
  }
  ...
  try {
    ...
    notification("Invite code copied to clipboard!", "#95ff8a");
    
    return referralLink;
  } catch (error) {
    console.error("Error creating referral link:", error);
    notification("Failed to create invite code. Please try again.", "#ff9999");
  }
}
